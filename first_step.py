                ...
                if player_count % 2 == 0:
                    ...
                    if checked_players_games:
                        ...
                        if time.time() - time_check >=20:
                            timeout = 5
                            print self.player_region +": Thread number " + str(self.threadID) + ": Press any key to end data collection"
                            rlist, wlist, xlist = select([sys.stdin], [], [], timeout)

                            if rlist:
                                print self.player_region +": Returning from thread "+str(self.threadID)
                                keep_running = False
                                break
                            else:
                                print self.player_region +": No key stroke detected.. Continuing data collection"
                    else:
